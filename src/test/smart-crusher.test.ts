...
describe("SmartCrusher", () => {
  test("reduces a 50-item array using Kneedle adaptive K", () => {
    const items = []
    for (let i = 0; i < 50; i++) {
      items.push({ id: i, name: `item-${i}`, value: i * 10 })
    }
    const content = JSON.stringify(items)
    const result = crushJsonArray(content, { min_items_to_analyze: 10, max_items: 30 })
  ...
  test("passthrough for arrays smaller than min_items_to_analyze", () => {
    const items = [{ id: 1 }, { id: 2 }, { id: 3 }]
    const content = JSON.stringify(items)
    const result = crushJsonArray(content, { min_items_to_analyze: 10 })
    expect(result).toBe(content)
  })

  test("keeps first and last items", () => {
    const items = []
    for (let i = 0; i < 100; i++) {
  ...
  test("falls through for non-JSON content", () => {
    const content = "This is not json at all"
    const result = crushJsonArray(content)
    expect(result).toBe(content)
  })

  test("falls through for non-array JSON", () => {
    const content = '{"key": "value"}'
    const result = crushJsonArray(content)
    expect(result).toBe(content)
  })

  test("appends omitted items comment", () => {
    const items = []
    for (let i = 0; i < 100; i++) {
  ...
  test("does not compress when item count = min_items_to_analyze", () => {
    const items = []
    for (let i = 0; i < 10; i++) {
      items.push({ id: i })
    }
  ...
  test("handles empty array", () => {
    const result = crushJsonArray("[]")
    expect(result).toBe("[]")
  })

  test("token monotone: does not expand small content when there's no value", () => {
    const items = []
    for (let i = 0; i < 20; i++) {
      items.push({ id: i })
    }
  ...
  test("CCR: injects marker and stores original when store is provided", () => {
    const items = []
    for (let i = 0; i < 100; i++) {
    ...
    const result = crushJsonArray(content, { min_items_to_analyze: 10 }, store)
  ...
  test("CCR: no marker when store is not provided", () => {
    const items = []
    for (let i = 0; i < 100; i++) {
      items.push({ id: i, name: `item-${i}`, value: i * 10, padding: "x".repeat(40) })
    }
  ...
  test("BM25 context: prioritizes context-relevant items in middle", () => {
    ...
    const content = JSON.stringify(items)
    const result = crushJsonArray(content, { min_items_to_analyze: 10 }, undefined, "needle haystack")
    const parsed = JSON.parse(result.split("\n//")[0])
  ...
  test("BM25 context: falls back to even sampling when no context match", () => {
    const items: { id: number; name: string }[] = []
    for (let i = 0; i < 50; i++) {
      items.push({ id: i, name: `item-${i}` })
    }
    const content = JSON.stringify(items)
    const result = crushJsonArray(content, { min_items_to_analyze: 10 }, undefined, "zephyr quantum")
